...
def read_seqinfo_file(seqinfo_file):
...
def main():
    ...
    for video_name in tqdm(video_names):
        ...
        seqinfo_file = osp.join(video_folder, 'seqinfo.ini')
        video_info = read_seqinfo_file(seqinfo_file)
        raw_img_folder = video_info['imDir']
        
        raw_img_names = os.listdir(osp.join(video_folder, raw_img_folder))
        raw_img_names = sorted(raw_img_names)
        num_raw_imgs = int(video_info['seqLength'])
        ...
        gts_file = osp.join(video_folder, 'gt/gt.txt')
        gts = []
        ...
        for gt_line in gts:
            ...
            frame_id, ins_id = map(int, gt_parts[:2])
            ltwh = list(map(float, gt_parts[2:6]))
            if is_mot15:
                class_id = 1
                visibility = 1.
            else:
                class_id = int(gt_parts[7])
                visibility = float(gt_parts[8])
            ...
            if frame_id != last_frame_id:
                ...
                raw_img = mmcv.imread(osp.join(video_folder, raw_img_folder, raw_img_name))
    ...
    with open(osp.join(reid_meta_folder, f'train_{int(100 * (1 - args.val_split))}.txt'), 'w') as f:
        f.writelines(reid_train_list)
    
    with open(osp.join(reid_meta_folder, f'val_{int(100 * args.val_split)}.txt'), 'w') as f:
