...
class MotChallenge2DBox(_BaseDataset):
    ...
    def __init__(self, config=None):
        ...
        for tracker in self.tracker_list:
            if self.data_is_zipped:
                curr_file = self.tracker_fol + '/' + tracker + '/' + self.tracker_sub_fol + '/' + seq + '.zip'
                if not os.path.isfile(curr_file):
                    print('Tracker file not found: ' + curr_file)
                    raise TrackEvalException('Tracker file not found: ' + tracker + '/' + os.path.basename(curr_file))
            else:
                for seq in self.seq_list:
                    curr_file = self.tracker_fol + '/' + tracker + '/' + self.tracker_sub_fol + '/' + seq + '.txt'
                    if not os.path.isfile(curr_file):
                        print('Tracker file not found: ' + curr_file)
    ...
    def _get_seq_info(self):
        ...
        if self.config["SEQ_INFO"]:
            ...
            if self.config["SEQMAP_FILE"]:
                ...
                # else:
                    seqmap_file = self.config["SEQMAP_FOLDER"][0] + '/' + self.gt_set + '.txt'
                    #seqmap_file = os.path.join(self.config["SEQMAP_FOLDER"], self.gt_set + '.txt').replace('/', os.sep)
            ...
            with open(seqmap_file) as fp:
                ...
                for i, row in enumerate(reader):
                    ...
                    seq = row[0]
                    seq_list.append(seq)
                    ini_file = self.gt_fol + '/' + seq + '/' + 'seqinfo.ini'
                    if not os.path.isfile(ini_file):
                        raise TrackEvalException('ini file does not exist: ' + self.gt_fol + '/' + seq + '/' + os.path.basename(ini_file) + '\n' + ini_file)
                    ini_data = configparser.ConfigParser()
                    ini_data.read(ini_file)
                    seq_lengths[seq] = int(ini_data['Sequence']['seqLength'])
